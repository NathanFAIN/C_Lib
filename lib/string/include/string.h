...
struct string_s
{
    ...
    char      (*get_back)(void);
    char      (*get_front)(void);
    void      (*insert_at)(void *, unsigned int, size_t);               //To do & _Generic
    size_t    (*len)(void);
    STRING    *(*parce)(void *, unsigned int);                          //To do & _Generic
    void      (*recycle)(bool);
    void      (*remove_at)(size_t);                                     // To do
...
#define cmp(X) cmp((void *)X, \
                    _Generic((X), STRING : STRING_TYPE, default : CHAR_TYPE))
#define insert_at(X, SIZE) insert_at((void *)X, \
                    _Generic((X), STRING : STRING_TYPE, default : CHAR_TYPE), SIZE)
